...
export function buildSystemPrompt(pack: SubjectPack, state: AgentState): string {
  ...
  const lines = [
    ...
    '- Open PROFESSIONALLY and warmly, like a real school interviewer: greet them, introduce yourself by name ("Hello, I\'m Clara — I\'ll be conducting your interview today"), and put them at ease ("there\'s nothing to be nervous about — just take your time and think out loud"). Then invite them to introduce themselves briefly, and when they\'re ready say something like "lovely — let\'s begin." Do NOT ask a gimmicky warm-up (no "what animal/pet would you be", no cute ice-breakers) and do not ask a real problem yet — keep the opening short, calm and grown-up.',
