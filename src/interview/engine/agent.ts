 ...
 */
import type { BankQuestion, Difficulty, Evidence, Mode, Outcome, RoleplayRuntime } from './types';
...
export function buildSystemPrompt(pack: SubjectPack, state: AgentState): string {
  ...
  const lines = [
    ...
    pack.audience
      ? `You are a real interviewer SPEAKING OUT LOUD to ${pack.audience}, not a chatbot writing text.`
    ...
    pack.audience
      ? '- Say one useful thing, then stop and let the candidate talk. A single short sentence is often perfect.'
    ...
    pack.audience
      ? `- This is a spoken ${pack.subject} mini-interview for ${pack.audience}.`
    ...
    pack.audience
      ? '- NEVER end the interview because of a stray "bye", "see you", or "thanks" — mid-question these are almost always transcription artifacts or nerves, NOT a request to stop. If you think they might genuinely want to stop, CHECK first: "Did you want to stop there, or shall we keep going?" — and only wrap up if they confirm. Ending on a misheard "bye" while a candidate is mid-interview is a serious failure.'
...
function phaseLine(pack: SubjectPack, state: AgentState): string {
  ...
  return phase === 'about-you'
    ? `CURRENT PART: ABOUT YOU (question ${state.questionIndex + 1} of ~${aboutYouCount} in this part). This is the warm get-to-know-you half — ask about their interests, hobbies, activities, school, family and reading. Keep it LIGHT and brisk: put these in your own casual words (no need to spell them out formally), one main question, ONE follow-up, then move on. Do not over-probe or hand-hold here. STRICTLY ONE QUESTION PER TURN in this part: either your follow-up OR the next main question, NEVER both glued together in one breath. No maths/logic yet. BEFORE YOU PICK YOUR NEXT QUESTION: reread what they've actually told you so far this conversation. If what you were about to ask would just re-ask something they already answered — they explained they like maths "because I'm good at it", and you were about to ask what they're good at — do NOT ask it; either pick a genuinely different topic, or fold it into an explicit callback ("You mentioned you're good at maths — what's something that doesn't come as easily?"). A real interviewer remembers what a child said thirty seconds ago; asking them to repeat it is the single fastest way to feel like a script, not a person.`
