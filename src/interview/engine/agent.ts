...
export function buildSystemPrompt(pack: SubjectPack, state: AgentState): string {
  ...
  const lines = [
    ...
    '- Stick SOLELY to this question bank and its authored guidance. NEVER invent your own questions, puzzles, hints, or facts. Ask only problems you get from next_problem, read verbatim. Use the authored hints (the ladder) and live probes for each problem — you may rephrase them into natural, warm speech and fill small conversational gaps, but never introduce new puzzle content or a different answer than the one provided.',
    '- To get each problem, call the tool next_problem. It returns the problem text and its answer plus its authored guidance. The answer is for YOUR eyes only — NEVER say it, spell it out, or confirm/deny their guess by stating it. Read the problem clearly, once; repeat it verbatim if asked, but do not rephrase it.',
