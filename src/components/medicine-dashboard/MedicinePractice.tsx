...
export function MedicinePractice({ onStartInterview }: Props) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      <div>
        <h1 style={{ fontFamily: "var(--med-display)", fontWeight: 700, fontSize: 30, margin: 0 }}>Practice</h1>
        <p style={{ color: 'var(--med-muted)', fontSize: 15, marginTop: 6 }}>Pick a real, published MMI format and run the full circuit with Clara.</p>
      </div>

      <div className="med-grid-two">
        {CIRCUITS.map((type) => (
          <div key={type.id} style={cardStyle}>
            <h3 style={{ fontFamily: "var(--med-display)", fontWeight: 700, fontSize: 20, margin: 0 }}>{type.name.replace('Medicine MMI — ', '')}</h3>
            <p style={{ color: 'var(--med-muted)', fontSize: 14.5, lineHeight: 1.6, marginTop: 10 }}>{type.description}</p>
            <div style={{ display: 'flex', gap: 16, marginTop: 16, fontSize: 13, color: 'var(--med-tertiary)' }}>
      ...
      <div style={cardStyle}>
        ...
        >
          <h3 style={{ fontFamily: "var(--med-display)", fontWeight: 700, fontSize: 17, margin: 0 }}>Question bank — what you'll be asked</h3>
          <span style={{ color: 'var(--med-primary-dark)', fontWeight: 600, fontSize: 13.5 }}>{showBank ? 'Hide' : 'Browse'} →</span>
        </button>
        {showBank && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 14, marginTop: 18 }}>
            {domains.map((d) => (
              <div key={d.id} style={{ background: 'var(--med-bg)', borderRadius: 12, padding: 16 }}>
                <div style={{ fontWeight: 600, fontSize: 14.5 }}>{d.label}</div>
                <div style={{ color: 'var(--med-tertiary)', fontSize: 13, marginTop: 4 }}>
