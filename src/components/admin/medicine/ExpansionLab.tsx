...
import { ActorBrief } from './ActorBrief';
...
import { MedicineTheme, useMedicineColourScheme, type MedicineColourScheme } from '@/components/medicine-dashboard/MedicineTheme';
...
import EngineRehearsal from './EngineRehearsal';
import GrowthWorkbench from './GrowthWorkbench';
import './expansion-workbench.css';
import './medicine-lab-extras.css';
...
export interface MedicineDraft {
  ...
  reviewRequired: string[]; originality: string; roleplay?: RoleplayRuntime;
}
...
export function expansionInfographicSvg(theme: MedicineColourScheme = 'clinical') {
...
export default function ExpansionLab({ standalone = false }: { standalone?: boolean }) {
  ...
  return (
    <MedicineTheme><div className={`med-lab ${standalone ? 'med-lab-standalone' : ''}`}>
      ...
      <nav className="lab-jump-nav" aria-label="Research desk sections"><a href="#build-updates">Latest updates</a><a href="#growth-workbench">Growth workbench</a><a href="/medicine/practice" target="_blank" rel="noreferrer">Student studio ↗</a><a href="#engine-rehearsal">Engine workbench</a><a href="#circuit-builder">Circuit builder</a><a href="#rehearsal">Draft review</a><a href="#content-coverage">Coverage & exports</a><a href="#school-evidence">School evidence</a>{isDev&&<a href="/__dev/medicine-design">Dashboard preview ↗</a>}</nav>
      ...
      <section className="lab-section lab-sources"><div className="lab-section-title"><h2>Source ledger</h2><span>CHECKED {research.updatedAt}</span></div>{research.sources.map((s) => <details key={s.id}><summary><span>{s.title}</span><ArrowUpRight size={15}/></summary><p>{s.supports}</p><p>{s.access} · checked {s.checked}</p><a href={s.url} target="_blank" rel="noreferrer">Open source ↗</a></details>)}</section>
