...
Deno.serve(async (req) => {
  ...
  // sessionId arrives as a query param (like token/pauseMs below) rather than a header, since the
  // browser WebSocket API can't set custom headers on the upgrade request — see the file-level
  // comment. Lets a relay-level error be tied back to the interview session it happened in.
  ...
  try {
    ...
    console.error("deepgram-relay error:", (err as Error)?.message || err);
    logAppEvent("edge:deepgram-relay", {
      level: "error",
