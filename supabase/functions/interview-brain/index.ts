...
const BANKS: Record<string, any> = {
  maths: mathsBank,
  logic: logicBank,
  currentaffairs: currentaffairsBank,
  elevenplus: elevenplusBank,
  medicine: medicineBank,
  chat: chatBank,
};
      ...
      return data
        ...
        .map((row: any) =>
          normalizeQuestionRow(
            row,
            (BANKS[row.subject] || []).find((q: any) => q.id === row.id),
          ),
        );
  } catch (e) {
    throw new Error(
      "Question bank unavailable; refusing to bypass database retirement controls.",
    );
  }
  ...
  withJson(async (req) => {
    ...
    try {
      ...
      if (
        ...
        return json({ error: "Valid sessionId and action are required" }, 400);
      if (
        body.studentText !== undefined &&
        (typeof body.studentText !== "string" ||
          body.studentText.length > 12000)
      )
        return json({ error: "Answer exceeds the turn limit" }, 400);
      if (
        body.topic !== undefined &&
        (typeof body.topic !== "string" || body.topic.length > 100)
      )
        return json({ error: "Invalid topic" }, 400);
      if (body.mode !== undefined && !["mock", "practice"].includes(body.mode))
        return json({ error: "Invalid practice mode" }, 400);
      if (
        body.expectedQuestionIndex !== undefined &&
        (!Number.isInteger(body.expectedQuestionIndex) ||
          body.expectedQuestionIndex < 0)
      )
        return json({ error: "Invalid station index" }, 400);
      if (
        body.turnId !== undefined &&
        (typeof body.turnId !== "string" ||
          !body.turnId.trim() ||
          body.turnId.length > 80)
      )
        return json({ error: "Invalid turn identifier" }, 400);
      ...
      const { data: session, error: sErr } = await admin
        ...
        .select(
          "id, user_id, interview_type, engine_state, engine_revision, status",
        )
        .eq("session_reference", sessionId)
        .maybeSingle();
      if (sErr)
        return json({ error: "Session service temporarily unavailable" }, 503);
      ...
      if (pilot) {
        ...
        if (body.mode === "practice" || action === "switch_topic")
          return json(
            { error: "This pilot follows a complete planned circuit" },
            400,
          );
      }
      ...
      if (body.turnId && session.engine_state?.lastTurn?.id === body.turnId) {
        ...
        return json({
          say: existing.lastTurn!.say,
          done: existing.done,
          uiState: uiStateOf(existing, pack, interviewTypeId),
        });
      }
      ...
      if (!session.engine_state) {
        if (action !== "start")
          return json({ error: "Start the interview first" }, 409);
        ...
        if (pilot) {
          const { data: history, error: historyError } = await admin
            ...
            .select("engine_state")
            .eq("user_id", userId)
            .like("interview_type", "medicine-%-pilot")
            .neq("id", session.id)
            .order("created_at", { ascending: false })
            .limit(30);
          if (historyError)
            return json(
              { error: "Practice history unavailable; please retry" },
              503,
            );
          const seenIds = (history || []).flatMap(
            (row: any) => row.engine_state?.askedIds || [],
          );
          const planned = initialiseMedicinePilot(
            pilot,
            deps.bank as PilotQuestion[],
            { seed: Math.floor(Math.random() * 2 ** 31), seenIds, studentName },
          );
          if (!planned.ok)
            return json(
              {
                error: `No fresh complete circuit: ${planned.shortages.join(", ")}. The draft pool needs more reviewed coverage.`,
              },
              409,
            );
          state = planned.state;
      ...
      if (body.turnId)
        result.state.lastTurn = { id: body.turnId, say: result.say };
      ...
      const { data: saved, error: saveError } = await admin
        ...
        .eq("id", session.id)
        .eq("status", "active")
        .eq("engine_revision", session.engine_revision)
        .select("id")
        .maybeSingle();
      if (saveError) throw saveError;
      if (!saved)
        return json(
          { error: "Another turn changed this session. Please retry." },
          409,
        );
      ...
      const response: BrainResponse = {
        ...
        uiState: uiStateOf(result.state, pack, interviewTypeId),
      };
